...
<form action="{{ route('admin.products.store') }}" method="post" enctype="multipart/form-data" id="productForm">
    ...
    <div class="mb-3">
        <label>Giá (VNĐ)</label>
        <input type="number" step="0.01" name="price" class="form-control" value="{{ old('price') }}" placeholder="Nhập giá sản phẩm">
        <small class="text-danger error-msg"></small>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-md-5">
            ...
            @php
                // Tìm attribute Màu sắc không phân biệt hoa thường (tránh lỗi do khác chữ hoa/thường)
                $colorsAttr = $attributes->first(function($a) {
                    return mb_strtolower($a->name) === mb_strtolower('Màu sắc');
                });
                $colorValues = $colorsAttr ? $colorsAttr->values : collect();
            @endphp

            @if($colorValues->isEmpty())
                <div class="text-muted">Chưa có giá trị cho thuộc tính "Màu sắc". Vui lòng thêm trong mục Giá trị thuộc tính.</div>
            @else
        ...
        <div class="col-md-5">
            ...
            @php
                // Tìm attribute Kích cỡ không phân biệt hoa thường
                $sizesAttr = $attributes->first(function($a) {
                    return mb_strtolower($a->name) === mb_strtolower('Kích cỡ');
                });
                $sizeValues = $sizesAttr ? $sizesAttr->values : collect();
            @endphp

            @if($sizeValues->isEmpty())
                <div class="text-muted">Chưa có giá trị cho thuộc tính "Kích cỡ". Vui lòng thêm trong mục Giá trị thuộc tính.</div>
            @else
                ...
                <div class="variant-item border p-3 mb-3 rounded position-relative">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-2">
                            ...
                            <input type="file" name="variant_images[${index}][]" class="form-control" accept="image/*" multiple>
                        </div>
